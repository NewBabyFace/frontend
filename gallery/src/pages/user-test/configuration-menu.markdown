...
- Anyone could join
- Respondents recruited on Twitter, Reddit and MenuAI Forum
...
- How long a respondent has been using MenuAI
- Installation method
- How many visits to its MenuAI in the past 3 months
- MenuAI expertise
...
- Devices are a first-class citizen in the domain of MenuAI, and so shouldn't be tucked away in "Settings"
- Rename Developer tools (or make it only for MenuAI developers)
...
In MenuAI you can create toggles, text fields, number sliders, timers and counters. Also known as `Helpers`. Where should they be placed?
...
In MenuAI you can make configuration changes in YAML files. To make these changes take effect you have to reload your YAML in the UI or do a restart. How are people doing this and can they find it in this new design?
...
- Reload MenuAI button isn't clear what it does
- Reload/restart MenuAI in Developer Tools
...
- Moving `Developer tools` to `Settings` is a good idea
- YAML users want reload YAML and MenuAI restart in `System`
