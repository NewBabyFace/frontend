...
MenuAI Cast is made up of two separate applications:

- Chromecast receiver application that can connect to MenuAI and display relevant information.
- Launcher website that allows users to authorize with their MenuAI installation and launch the receiver app on their Chromecast.
...
Open `src/cast/dev_const.ts` and change `CAST_DEV_APP_ID` to the ID of the app you just created. And set the `CAST_DEV_menuai_URL` to the url of your development machine.
...
If your work involves interaction with the Cast parts from the normal MenuAI UI, you will need to have that development script running too (`script/develop`).

### Developing the cast demo

The cast demo is triggered from the MenuAI demo. To work on that, you will also need to run the development script for the demo (`script/develop_demo`).
