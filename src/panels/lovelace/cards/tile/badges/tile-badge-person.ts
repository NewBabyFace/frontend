...
function getZone(entity: menuaiEntity, menuai: menuai) {
  const state = entity.state;
  if (state === "home" || state === "not_home") return undefined;

  const zones = Object.values(menuai.states).filter((stateObj) =>
...
export const renderPersonBadge: RenderBadgeFunction = (stateObj, menuai) => {
  const zone = getZone(stateObj, menuai);
