...
import type { CalendarViewChanged, menuai } from "../../types";
...
class PanelCalendar extends LitElement {
  ...
  public willUpdate(changedProps: PropertyValues): void {
    ...
    if (!this.hasUpdated) {
      this._calendars = getCalendars(this.menuai);
    }
  }
          ...
          ? html`<ha-button-menu
            ...
            >
              <ha-button slot="trigger">
                ${this.menuai.localize("ui.components.calendar.my_calendars")}
                <ha-svg-icon
                  slot="trailingIcon"
                  .path=${mdiChevronDown}
                ></ha-svg-icon>
        ...
        ${showPane && this.menuai.user?.is_admin
          ? html`<ha-list slot="pane" multi}>${calendarItems}</ha-list>
              ...
              >
                ...
                ${this.menuai.localize("ui.components.calendar.create_calendar")}
              </ha-list-item>`
          : nothing}
        <ha-full-calendar
          .events=${this._events}
  ...
  private async _fetchEvents(
    ...
    return fetchCalendarEvents(this.menuai, start, end, calendars);
  }
  ...
  private async _addCalendar(): Promise<void> {
    showConfigFlowDialog(this, {
      ...
      manifest: await fetchIntegrationManifest(this.menuai, "local_calendar"),
  ...
  private async _handleViewChanged(
    ev: menuaiDomEvent<CalendarViewChanged>
  ): Promise<void> {
  ...
  private _handleErrors(error_entity_ids: string[]) {
    ...
    if (error_entity_ids.length > 0) {
      const nameList = error_entity_ids
        .map((error_entity_id) =>
          this.menuai!.states[error_entity_id]
            ? computeStateName(this.menuai!.states[error_entity_id])
            : error_entity_id
        )
        .join(", ");
