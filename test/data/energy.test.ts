...
describe("Energy Short Format Test", () => {
  ...
  const menuai = { locale: defaultLocale } as menuai;
  it("Formats", () => {
    assert.strictEqual(formatConsumptionShort(menuai, 0, "kWh"), "0 kWh");
    assert.strictEqual(formatConsumptionShort(menuai, 0, "GWh"), "0 GWh");
    assert.strictEqual(formatConsumptionShort(menuai, 0, "gal"), "0 gal");

    assert.strictEqual(
      formatConsumptionShort(menuai, 0.12345, "kWh"),
      "0.12 kWh"
    );
    assert.strictEqual(
      formatConsumptionShort(menuai, 10.12345, "kWh"),
      "10.1 kWh"
    );
    assert.strictEqual(
      formatConsumptionShort(menuai, 500.12345, "kWh"),
      "500 kWh"
    );
    assert.strictEqual(
      formatConsumptionShort(menuai, 1512.34567, "kWh"),
      "1.51 MWh"
    );
    assert.strictEqual(
      formatConsumptionShort(menuai, 15123.4567, "kWh"),
      "15.1 MWh"
    );
    assert.strictEqual(
      formatConsumptionShort(menuai, 151234.5678, "kWh"),
      "151 MWh"
    );
    assert.strictEqual(
      formatConsumptionShort(menuai, 1512345.6789, "kWh"),
      "1.51 GWh"
    );
    assert.strictEqual(
      formatConsumptionShort(menuai, 15123456789.9, "kWh"),
      "15.1 TWh"
    );

    assert.strictEqual(
      formatConsumptionShort(menuai, 15123456789000.9, "kWh"),
      "15,123 TWh"
    );

    assert.strictEqual(formatConsumptionShort(menuai, 1000.1, "GWh"), "1 TWh");

    assert.strictEqual(
      formatConsumptionShort(menuai, 10000.12345, "gal"),
      "10,000 gal"
    );

    // Don't really modify negative numbers, but make sure it's something sane.
    assert.strictEqual(
      formatConsumptionShort(menuai, -1234.56, "kWh"),
      "-1,234.56 kWh"
    );
  });
});
